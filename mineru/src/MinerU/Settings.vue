...
<template>
  <main class="mineru-page">
    <section class="mineru-card">
      ...
      <p class="message">配置 MinerU 远程解析服务的 API 端点和密钥。</p>

      <div class="form-group">
        <label for="endpoint">API 端点</label>
        <input id="endpoint" v-model.trim="config.endpoint" type="url" placeholder="https://mineru.net/api/v4" :class="{ 'input-error': endpointError }" />
        <p v-if="endpointError" field-error>{{ endpointError }}</p>
      </div>

      <div class="form-group">
        <label for="apiKey">API Key</label>
        <input id="apiKey" v-model.trim="config.apiKey" type="password" placeholder="输入 API Key（可选）" />
        <p class="hint">部分接口（如标准模式）需要 API Key，Flash 模式可留空。</p>
      </div>
